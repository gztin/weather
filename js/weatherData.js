...
function loadData(){
        ...
        function showPosition(position) {
            ...
            var aaa = position.coords.latitude+','+position.coords.longitude;
            $('.yourPosition').text(aaa);
            var pos =  $('.yourPosition').text();
            console.log('位置是'+pos);
             // 取得所在位置
            fetch('https://api.map8.zone/v2/place/geocode/json?key=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..GOwnExWzTlfc5wN1H7a3bX3uegL6I3DO34XNSUU55Z0&latlng='+pos, {})
            .then((response) => {
                    // 可以透過 blob(), json(), text() 轉成可用的資訊
                    return response.json(); 
                }).then((placeData) => {
                    console.log(placeData);
                    var dataPlace = placeData.results[0].city;
                    console.log('城市為:'+dataPlace);
                    
                    if(dataPlace =='嘉義縣'){
                        var cityNumber = 0;   
                    }else if(dataPlace =='新北市'){
                        var cityNumber = 1;   
                    }else if(dataPlace =='嘉義市'){
                        var cityNumber = 2;   
                    }else if(dataPlace =='新竹縣'){
                        var cityNumber = 3;   
                    }else if(dataPlace =='新竹市'){
                        var cityNumber = 4;   
                    }else if(dataPlace =='台北市'){
                        var cityNumber = 5;   
                    }else if(dataPlace =='台南市'){
                        var cityNumber = 6;   
                    }else if(dataPlace =='宜蘭縣'){
                        var cityNumber = 7;   
                    }else if(dataPlace =='苗栗縣'){
                        var cityNumber = 8;   
                    }else if(dataPlace =='雲林縣'){
                        var cityNumber = 9;   
                    }else if(dataPlace =='花蓮縣'){
                        var cityNumber = 10;   
                    }else if(dataPlace =='台中市'){
                        var cityNumber = 11;   
                    }else if(dataPlace =='台東縣'){
                        var cityNumber = 12;   
                    }else if(dataPlace =='桃園市'){
                        var cityNumber = 13;   
                    }else if(dataPlace =='南投縣'){
                        var cityNumber = 14;   
                    }else if(dataPlace =='高雄市'){
                        var cityNumber = 15;   
                    }else if(dataPlace =='金門縣'){
                        var cityNumber = 16;   
                    }else if(dataPlace =='屏東縣'){
                        var cityNumber = 17;   
                    }else if(dataPlace =='基隆市'){
                        var cityNumber = 18;   
                    }else if(dataPlace =='澎湖縣'){
                        var cityNumber = 19;   
                    }else if(dataPlace =='彰化縣'){
                        var cityNumber = 20;   
                    }else{
                        var cityNumber = 21;   
                    }
